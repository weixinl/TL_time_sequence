...
class Domain_Branch(nn.Module):
    def __init__(self,_domain_num,_device):        
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=9, out_channels=16, kernel_size=(1, 9)),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(1, 2), stride=2)
        )
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(in_features=4*28, out_features=32),
            nn.ReLU()
        )
        self.fc2= nn.Sequential(
            nn.Linear(in_features=32, out_features=_domain_num),
            nn.ReLU()
        )
...
class Transfer_Decoder(nn.Module):
    def __init__(self):
        ...
        self.convtranspose2=nn.Sequential(
            ...
            nn.ConvTranspose2d(in_channels=32, out_channels=9, kernel_size=(1, 9)),
            nn.ReLU()
        )
...
class Base_Label_Extractor(nn.Module):
    def __init__(self,_label_cnt):
        super(Base_Label_Extractor,self).__init__()
        self.fc1 = nn.Sequential(
            nn.Linear(in_features=64 * 26, out_features=_label_cnt),
            nn.ReLU()
        )
...
class Label_Subbranch(nn.Module):
    def __init__(self,_label_num,_device):
        super(Label_Subbranch,self).__init__()
        self.encoder=Transfer_Encoder().to(_device)
        self.label_extractor=Base_Label_Extractor(_label_num).to(_device)
        # self.reconstruct_loss_obj=nn.MSELoss()
...
class Label_Branch_With_Reconstruct(nn.Module):
    ...
    def forward(self,_in,_domain_prob_vec_by_entry):
        ...
        weighted_sum_encoded_layer_by_entry=torch.zeros([batch_size,64,1,26],dtype=float).to(self.device)
        weighted_sum_label_vec_by_entry=torch.zeros([batch_size,self.label_num],dtype=float).to(self.device)
        # weighted_sum_reconstruct_loss=torch.zeros([1],dtype=float).to(self.device)
